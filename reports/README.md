...
These commands allow for easy execution of each stage in an isolated and reproducible environment. The use of Docker ensured consistency across different systems and simplified the setup process for our experiments.
...
For unit testing, we used pytest to ensure the functionality of individual components in our API. The unit tests focused on verifying the correctness of input validation, expected output responses, and edge cases within the API endpoints. These tests were integrated into the CI/CD pipeline using GitHub Actions, enabling automated testing with each code push.

For load testing, we used Locust to evaluate the performance and scalability of the API under varying levels of concurrent user requests. We simulated multiple scenarios, including low to high traffic, by gradually increasing the number of users sending requests to the FastAPI endpoint deployed on Google Cloud Run.
...
--- 
The diagram illustrates the architecture of a system designed for deploying and monitoring machine learning models in both local and cloud environments. It leverages various tools and services to ensure efficient development, testing, deployment, and monitoring.

System Description:

Local Setup:

Development begins in a local environment where Git is used for version control, and Python code is structured following a Cookiecutter project template.
The local setup integrates Hydra for configuration management, PyTorch for building and training machine learning models, and tools like Profiler & Debugger for performance optimization.
Experiment tracking is enabled using Weights & Biases (W&B), while Conda handles package management and environment creation.
Data Management:

Data versioning is managed using DVC, which interacts with a GCP Bucket for remote storage. This setup facilitates the synchronization of training, predicted, and telemetry data between local and cloud environments.
Cloud Integration:

Code pushed to GitHub triggers GitHub Actions for continuous integration. This includes running tests using pytest and performing timing checks.
Dockerized applications are built and stored in an Artifact Registry, which is later deployed to Google Cloud Run for scalable API hosting.
GCP Compute Engine is used for resource-intensive computations.
API Deployment and Monitoring:

The deployed API is built with FastAPI, which handles user requests.
Load testing is conducted using Locust, and monitoring is performed using Prometheus, which tracks data drift and system telemetry.
Workflow Automation:

A cloud build trigger ensures seamless integration between the local and cloud environments. Predictions generated by the system are stored in the GCP Bucket for further analysis and monitoring.
In summary, the system integrates local development tools with cloud-based services to create a robust, scalable architecture for deploying and monitoring machine learning models. It ensures data versioning, continuous integration, automated deployment, and comprehensive monitoring.
![SLOs for Cloud Run](figures/myOverview.png)
