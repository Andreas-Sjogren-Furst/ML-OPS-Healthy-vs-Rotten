...
def train(cfg: DictConfig) -> None:
    ...
    wandb.init(project=cfg.wandb.project, entity=cfg.wandb.entity, config=config, mode=cfg.wandb.mode)
    ...
    if cfg.training.small_dataset:
        ...
        train_indices = random.sample(
            train_healthy_idx, int(np.floor(cfg.training.small_dataset_proportion * len(train_healthy_idx)))
        ) + random.sample(
            train_rotten_idx, int(np.floor(cfg.training.small_dataset_proportion * len(train_rotten_idx)))
        )
        val_indices = random.sample(
            val_healthy_idx, int(np.floor(cfg.training.small_dataset_proportion * len(val_healthy_idx)))
        ...
        wandb.log(
            {
                "val/confusion_matrix": wandb.plot.confusion_matrix(
                    probs=None, y_true=all_labels, preds=all_predictions, class_names=["Rotten", "Healthy"]
                )
            }
        )
        ...
        if val_loss < best_val_loss:
            ...
            if cfg.training.save_model:
                ...
                artifact = wandb.Artifact(
                    name=f"model-{wandb.run.id}", type="model", description=f"Best model from run {wandb.run.name}"
                )
